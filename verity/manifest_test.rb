...
test "manifest claim_next returns nil when all pending tests are excluded" do
  manifest = Verity::Manifest.open(":memory:").tap(&:migrate!)
  lone = MMAKE.call(fingerprint: "a.rb:1111111111111111")
  manifest.replace_tests([lone])
  begin
    claimed = manifest.claim_next(1, exclude: [lone.fingerprint])
    assert_equal actual: claimed, expected: nil
  ensure
    manifest.close
  end
end

test "manifest claim_next with exclude skips the listed fingerprints" do
  manifest = Verity::Manifest.open(":memory:").tap(&:migrate!)
  first = MMAKE.call(fingerprint: "a.rb:1111111111111111")
  second = MMAKE.call(fingerprint: "b.rb:2222222222222222")
  manifest.replace_tests([first, second])
  begin
    claimed = manifest.claim_next(1, exclude: [first.fingerprint])
    assert_equal actual: claimed.fingerprint, expected: second.fingerprint
  ensure
    manifest.close
  end
end

test "manifest running_resources returns resource hashes for running rows only" do
  manifest = Verity::Manifest.open(":memory:").tap(&:migrate!)
  running = MMAKE.call(
    fingerprint: "r.rb:1111111111111111",
    resources: { tables: [:users, :posts] }
  )
  pending = MMAKE.call(
    fingerprint: "p.rb:2222222222222222",
    resources: { tables: [:orders] }
  )
  manifest.replace_tests([running, pending])
  manifest.claim_next(1)
  begin
    result = manifest.running_resources
    assert_equal actual: result, expected: [{ "tables" => ["users", "posts"] }]
  ensure
    manifest.close
  end
end

test "manifest running_resources returns empty when no rows are running" do
  manifest = Verity::Manifest.open(":memory:").tap(&:migrate!)
  begin
    assert_equal actual: manifest.running_resources, expected: []
  ensure
    manifest.close
  end
end
